...
class NoteService:
    ...
    async def capture(self, user_key: str, content: MessageContent) -> CaptureResult:
        prompt = routing_system(self._vault.list_folders(user_key), INBOX_FOLDER, self._existing(user_key))
        header, body = self._parse(await self._llm.complete(prompt, content))
        title = header.title.strip() or DEFAULT_TITLE

        target = self._vault.find_note(user_key, header.update_of.strip())
        ...
        folder = header.folder if header.folder in set(folders) | {INBOX_FOLDER} else INBOX_FOLDER
    ...
    @staticmethod
    def _parse(raw: str) -> tuple[NoteDraftHeader, str]:
        parts = raw.split(BODY_SEPARATOR, 1)
        header_raw = JSON_FENCE.sub("", parts[0]).strip()
        body = parts[1].strip() if len(parts) > 1 else ""
        try:
            header = NoteDraftHeader.model_validate_json(header_raw)
